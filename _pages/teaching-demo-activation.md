...
demo_badge: "Demo 2.5 · Activation functions"
...
This demo extends the scalar-accumulation walk (Demo 2) with one more row: the pre-activation `z = w₁x₁ + w₂x₂ + b` is passed through a switchable activation function `φ` to produce the neuron's actual output `a = φ(z)`.
