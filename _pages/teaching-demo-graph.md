...
demo_badge: "Demo 5 · Computational graphs & autodiff"
...
Demo 6 shows a gradient as a matrix heatmap; this demo shows where that number actually comes from, one scalar operation at a time. You build a small graph of `+ − × ÷` nodes from dropdown menus, and every wire in the graph carries a value forward (black, pointing right) and a local derivative backward (blue, pointing left).
